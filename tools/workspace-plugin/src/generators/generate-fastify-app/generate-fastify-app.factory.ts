...
export function generateFastifyApp(schema: GenerateFastityAppSchema): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    const appFilesSource = apply(url('./files/app'), [
      template({
    ...
    const mainFileSource = apply(url('./files/main.ts'), [
      template({
        ...strings, // This is were dasherize and classify etc. come from
        ...schema,
      }),
    ...
    const rule = chain([mergeWith(appFilesSource), mergeWith(mainFileSource)]);
